...
namespace PekatPortal.Module.BusinessObjects
{
    ...
    public class vw_RDR1 : XPLiteObject
    { // Inherit from a different class to provide a custom primary key, concurrency and deletion behavior, etc. (https://documentation.devexpress.com/eXpressAppFramework/CustomDocument113146.aspx).
        public vw_RDR1(Session session)
            : base(session)
        {
        }
        ...
        private vw_ORDR _DocEntry;
        [XafDisplayName("Order Detail")]
        [Association("vw_ORDR-vw_RDR1")]
        [Appearance("_vw_ORDR", Enabled = false)]
        [Browsable(true)]
        [Appearance("DocEntry", Enabled = false)]
        [VisibleInListView(false), VisibleInDetailView(false), VisibleInLookupListView(false)]
        public vw_ORDR DocEntry
        {
